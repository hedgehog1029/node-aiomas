...
class Payload {
    ...
    readFrom(target) {
        this.pointer += target.copy(this.buf, this.pointer, 0, this.len - this.pointer);
    }
}
...
class ChannelClient {
    constructor(address, opts) {
        ...
        const createSock = () => {
            ...
            sock.on('data', (buf) => this._onRawData(buf));
    ...
    _onRawData(buf) {
        ...
        while (buf.length > 0) {
            ...
            payload.readFrom(buf);
            buf = buf.slice(payload.len);
